  ...
  return (
    <div className="flex flex-col gap-6">
      <header className="flex flex-col gap-3">
        ...
        <p className="text-sm text-zinc-600">
          Prepared for{" "}
          <span className="font-medium text-zinc-950">{project.client}</span>
        </p>
        {project.notes && (
          <p className="text-sm text-zinc-600 whitespace-pre-line">
            {project.notes.replace("__demoSeeded", "").trim()}
          </p>
        )}
      </header>
        ...
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {items.map((item: Item) => (
            <Link
              ...
              className="group flex flex-col overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-card hover:shadow-lg transition"
            >
              <div className="relative h-40 w-full overflow-hidden bg-zinc-100">
                ...
                {item.images && item.images.length > 0 ? (
                  <img
                    src={item.images[item.previewImageIndex] ?? item.images[0]}
                    alt={item.name}
              ...
              <div className="flex flex-1 flex-col gap-2 p-4">
                <p className="text-sm font-semibold text-zinc-950 line-clamp-2">
                  {item.name}
                </p>
